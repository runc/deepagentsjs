...
export class IndexedDBCheckpointSaver extends BaseCheckpointSaver {
  private async migratePendingSends(
    ...
    const writes = (await db.get("checkpoint_writes", parentKey)) ?? {};
  ...
  async getTuple(config: RunnableConfig): Promise<CheckpointTuple | undefined> {
    ...
    if (threadId && checkpointId) {
      ...
      const saved = await db.get("checkpoints", key);
      if (!saved) {
        return undefined;
      }
    ...
    const allKeys = await db.getAllKeys("checkpoints");
    ...
    const saved = await db.get("checkpoints", latest.key);
    if (!saved) {
      return undefined;
    }
  ...
  async *list(
    ...
    for (const threadId of threadIds) {
      const namespaces = [
        ...new Set(
          parsedKeys.filter(({ parsed }) => parsed.threadId === threadId).map(({ parsed }) => parsed.checkpointNamespace),
        ),
      ];
      ...
      for (const checkpointNamespace of namespaces) {
        ...
        for (const { key, parsed } of checkpoints) {
          if (configCheckpointId && parsed.checkpointId !== configCheckpointId) {
            continue;
          }
          if (
  ...
  async putWrites(
    ...
    const existing = (await db.get("checkpoint_writes", outerKey)) ?? {};
    ...
    await db.put("checkpoint_writes", existing, outerKey);
  }

  async deleteThread(threadId: string): Promise<void> {
    assertSafeStorageKey("thread_id", threadId);
    const db = await getDocAgentDb();
    const checkpointKeys = await db.getAllKeys("checkpoints");
    for (const key of checkpointKeys) {
      if (parseCheckpointKey(key).threadId === threadId) {
        await db.delete("checkpoints", key);
        await db.delete("checkpoint_writes", key);
      }
    }
  }
  ...
  private async loadPendingWrites(key: string): Promise<CheckpointPendingWrite[]> {
    const db = await getDocAgentDb();
    const writes = (await db.get("checkpoint_writes", key)) ?? {};
    return Promise.all(
...
let sharedCheckpointer: IndexedDBCheckpointSaver | null = null;

export function getDocAgentCheckpointer(): IndexedDBCheckpointSaver {
  if (!sharedCheckpointer) {
    sharedCheckpointer = new IndexedDBCheckpointSaver();
  }
